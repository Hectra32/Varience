...
void panel_update(Game *game) {
  ...
  if ((game->mouse_x >= game->panel.x + 100) &&
      ...
      (game->mouse_y <= game->panel.y + 50 + 50) && (game->mouse_left_one_f)) {
    add_object(AND);
  }
...
void draw_panel_object(Game *game) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    int x = game->panel.x + 100 + col * 200;
    int y = game->panel.y + 50 + row * 100;

    SDL_Rect object = {x, y, gate[i].width,
                       gate[i].height}; // Gave it Propeer Difination

    SDL_SetRenderDrawColor(game->renderer, gate[i].color[0], gate[i].color[1],
                           gate[i].color[2],
                           gate[i].color[3]); // Moved It Avove Drawing Text

    SDL_RenderFillRect(game->renderer, &object); // Draw Object Rect

    draw_text_center(game->renderer, game->font, gate[i].NAME, x, y,
                     gate[i].width, gate[i].height,
                     (SDL_Color){255, 255, 255, 252}); // Drawing Text
  }
}
...
void panel_draw(Game *game) {
  ...
  SDL_Rect panel = {game->panel.x, game->panel.y, game->panel.w, game->panel.h};
